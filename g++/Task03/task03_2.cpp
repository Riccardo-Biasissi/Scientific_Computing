...
std::vector<double> read_vector(const std::string& filename) {
    std::vector<double> vec;        // Create an empty vector
    std::ifstream file(filename);   // Open the file for reading
    double value;                   // Create a double to store each value
...
void calculate_d(const std::string& config_file) {
    ...
    std::vector<double> x = read_vector(filename_x);
    std::vector<double> y = read_vector(filename_y);
...
// g++ task03_2.cpp -o task03_2
// ./task03_2 config.txt
//
// Where the config.txt file contains the following configuration:
...
//
// And the x.dat and y.dat files are the input vectors. Note that the 
// config.txt file should not contain any spaces around the '=' sign. This
// is because of how the read_config function bas been implemented.