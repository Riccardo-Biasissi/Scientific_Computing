...
// and saves them to files with the given filename_prefix (SERIAL VERSION)
void generate_vectors(int N, std::string filename_prefix) {
    std::cout << "Generating vectors of size " << N << " using SERIAL version (1 thread)" << std::endl;
    ...
    std::cout << "SERIAL vector generation time: " << gen_duration.count() << " ms" << std::endl;
    
    // Create filenames using the filename_prefix and N
    std::string filename_x = filename_prefix + "N" + std::to_string(N) + "_x_serial.dat";
    std::string filename_y = filename_prefix + "N" + std::to_string(N) + "_y_serial.dat";
    ...
    std::cout << "SERIAL total operation time: " << total_duration.count() << " ms" << std::endl;
}
