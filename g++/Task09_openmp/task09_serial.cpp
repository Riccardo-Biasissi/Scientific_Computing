...
void calculate_d(const std::string& config_file, int chunksize = 16) {
    ...
    // // Print the partial sums for each chunk (only first 10 for large datasets)
    // std::cout << "Partial sums for each chunk (first 10): [";
    // int print_limit = std::min(10, chunk_number);
    // for (int chunk = 0; chunk < print_limit; ++chunk) {
    //     std::cout << partial_chunk_sum[chunk];
    //     if (chunk != print_limit - 1) std::cout << " ";
    // }
    // if (chunk_number > 10) std::cout << " ...";
    // std::cout << "]" << std::endl;
