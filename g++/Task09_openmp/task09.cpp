...
void calculate_d(const std::string& config_file, int chunksize = 16) {
    ...
    // Print the number of chunks and OpenMP threads
    ...
    // Calculate vector d progressing in chunks with OpenMP parallelization
    ...
    // Compute the total sum of vector d with OpenMP parallelization
    long double d_complete_sum = 0.0;
    #pragma omp parallel for reduction(+:d_complete_sum)
    for (int i = 0; i < N; ++i) {
        d_complete_sum += d[i];
    }
